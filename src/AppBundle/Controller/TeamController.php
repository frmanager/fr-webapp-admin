...
class TeamController extends Controller
{
  ...
  public function teamIndexAction($campaignUrl)
  {
     ...
     * @Method({"GET", "POST"})
     */
    public function reassignAction(Request $request, $campaignUrl, $teamUrl)
    {
      $logger = $this->get('logger');
      $em = $this->getDoctrine()->getManager();
      ...
      $newType = $em->getRepository('AppBundle:TeamType')->findOneByValue($type);
      $team->setTeamType($newType);

      if($type == "teacher" || $type == "student"){
        $teamStudents = $em->getRepository('AppBundle:TeamStudent')->findByTeam($team);
