...
class DonationController extends Controller
{
    ...
    public function indexAction($campaignUrl, LoggerInterface $logger)
    {
      ...
      if(!$campaignHelper->campaignPermissionsCheck($this->get('security.token_storage')->getToken()->getUser(), $campaign)){
        ...
        $donations = $em->getRepository('App:Donation')->findByCampaign($campaign);
    ...
    public function newAction(Request $request, $campaignUrl, LoggerInterface $logger)
    {
      ...
      if ($request->isMethod('POST')) {
          ...
          if(!empty($params['setClassroomFlag'])){
            $donation->setClassroom($em->getRepository('App:Classroom')->find($params['donation']['classroomID']));
          }

          if($type == 'team'){
            if(null !== $params['donation']['teamID']){
              $team = $em->getRepository('App:Team')->find($params['donation']['teamID']);
              if(is_null($team)){
            ...
            if(null !== $params['donation']['classroomID']){
              $classroom = $em->getRepository('App:Classroom')->find($params['donation']['classroomID']);
            ...
            if(null !== $params['donation']['studentID']){
              $student = $em->getRepository('App:Student')->find($params['donation']['studentID']);
              if(is_null($student)){
      ...
      return $this->render('donation/donation.form.html.twig', array(
          'donation' => $donation,
          'campaign' => $campaign,
          'teams' => $em->getRepository('App:Team')->findBy(array('campaign' => $campaign)),
          'type' => $type,
          'classrooms' => $em->getRepository('App:Classroom')->findBy(array('campaign' => $campaign), array('grade' => 'asc', 'name'=>'asc'))
      ));
     ...
     * @Method({"GET", "POST"})
     */
    public function reassignAction(Request $request, $campaignUrl, $donationID, LoggerInterface $logger)
    {
      ...
      $donation = $em->getRepository('App:Donation')->find($donationID);
      if(is_null($donation)){
        $this->get('session')->getFlashBag()->add('warning', 'We are sorry, we could not find this campaign.');
        return $this->redirectToRoute('donation_index', array('campaignUrl'=>$campaign->getUrl()));
      }
          ...
          if(!empty($params['setClassroomFlag'])){
            if("" == $params['donation']['classroomID'] || null == $params['donation']['classroomID']){
              ...
              if(is_null($classroom)){
                ...
                $donation->setClassroom($em->getRepository('App:Classroom')->find($classroom->getId()));
              }
            }
          }
     ...
     * @Method({"GET", "POST"})
     */
    public function editAction(Request $request, $donationID, LoggerInterface $logger)
    {
      ...
      //CODE TO CHECK TO SEE IF DONATION EXISTS
      $donation = $em->getRepository('App:Donation')->findOneBy(array('id'=>$donationID, 'campaign' => $campaign));
      if(is_null($donation)){
     ...
     * @Method("DELETE")
     */
    public function deleteAction(Request $request, Donation $donation, LoggerInterface $logger)
    {
        $entity = 'Donation';
     ...
     * @Method({"GET", "POST"})
     */
    public function verifyStudentAction(Request $request, $campaignUrl, $donationID, LoggerInterface $logger)
    {
            ...
            if ($file['file']->getClientOriginalExtension() == 'csv') {
                ...
                if ($CSVHelper->validateHeaders($templateFields)) {
                    ...
                    foreach ($CSVHelper->getData() as $i => $item) {
                        ...
                        $student = $em->getRepository('App:Student')->find($item['student_id']);
                        if (empty($student)) {
                            $rowFailure = true;
                            $fileFailed = true;
                        ...
                        if (!$rowFailure) {
                            $donation = $this->getDoctrine()->getRepository('App:Donation')->findOneBy(
    ...
    public function fileLoadAction(Request $request, $campaignUrl, LoggerInterface $logger)
    {
